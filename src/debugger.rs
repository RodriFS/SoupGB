...
fn print_instruction(instruction: u8, memory: &Memory) {
    ...
    match instruction {
        ...
        0xe0 => println!("LD (0xFF00 + 0x{:X}), A", memory.read(pc + 1)), // 0xe0
        ...
        0xf0 => println!("LD A, (0xFF00 + 0x{:X})", memory.read(pc + 1)), // 0xf0
...
pub fn print_debug_registers_info(debug: bool, registers: &Registers) {
    if !DEBUG_CPU || !debug {
        return;
    }
    let flags = (
...
pub fn print_debug_memory_info(debug: bool, memory: &Memory, timers: &Timers) {
    if !debug {
        return;
    }
    if DEBUG_CPU {
