    ...
    if request > 0 && emu.timers.master_enabled {
        for bit in 0..5 {
            if get_bit_at(request, bit) && get_bit_at(interrupt_enable, bit) {
                ...
                let cancelled = interrupt_execution(emu, request, bit);
                if !cancelled {
                    break;
                }
            }
...
pub fn interrupt_execution(emu: &mut Emulator, request: u8, interrupt: u8) -> bool {
    ...
    if !get_bit_at(interrupt_enable, interrupt) {
        emu.take_cycle();
        emu.memory.set_program_counter(0);
        true
    } else {
        ...
        emu.take_cycle();
        emu.memory.set_program_counter(new_pc as u16);
        false
    }
}
